...
void main()
{
    ...
    set_registers();
    gpio_config_io();
    send_packet_io37(1); // configuration finished

    for (j=0;j<9;j++){
        send_packet_io37(j+2); // send 4 pulses at gpio[j]
    ...
    send_packet_io37(1); // reset counter
    for (j=9;j<19;j++){
        send_packet_io37(j-9+2); // send 4 pulses at gpio[j]
        for (i = 0; i < num_pulses; i++){
            reg_mprj_datal = 0x1 << j;
            count_down(PULSE_WIDTH);  
            reg_mprj_datal = 0x0;  
    ...
    send_packet_io37(1); // finish test
    send_packet_io37(1); // finish test
    send_packet_io37(1); // finish test
