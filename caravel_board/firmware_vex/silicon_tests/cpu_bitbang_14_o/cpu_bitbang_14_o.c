...
/*
    configure the gpio[14] as mgmt output using bitbang then send the next pattern on them 
    generate send 10 pulses in the gpio 14 
    gpio_37 10 10 
    @ before bit bang
        send packet size = 1 
    @after bitbang 
        send packet size = 2
    @ start sending 10 pulses at gpio[14] 
        send packet size = 5 
    @ stop sending pulses at gpio[14] 
        send packet size = 7
    @ finish test 
        send packet size = 3 
        send packet size = 3 
...
void main(){
    ...
    send_packet(5);// start sending 10 pulses at gpio[14] 
    ...
    send_packet(7);// stop sending 10 pulses at gpio[14] 
