...
/*
    configure the gpio[37] as mgmt output using bitbang then send the next pattern on them 
    ...
    @after bitbang 
        send packet size = 2
    @ start sending 10 pulses at gpio[37] 
        send packet size = 5 
    @ stop sending pulses at gpio[37] 
        send packet size = 7
    @ finish test 
        send packet size = 3 
        send packet size = 3 
...
void main(){
    ...
    send_packet(5);// start sending 10 pulses at gpio[37] 
    ...
    send_packet(7);// stop sending 10 pulses at gpio[37] 
