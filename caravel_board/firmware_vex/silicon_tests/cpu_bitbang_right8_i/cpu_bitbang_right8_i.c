...
// --------------------------------------------------------
/*
    configure the gpio[37:30] as mgmt input using bitbang then waiting for the following patterns in order
    0xAA 
    0x55  
    0xFF 
    0x00
...
void wait_for_pattern(int pattern){
    ...
    for (int i = 0; i < time_out; i++){
        if ( reg_mprj_datah == pattern>>2){
            send_packet(7); // found pattern
            is_found = true;
        }
    }
    if (!is_found){
...
void main()
{
    ...
    wait_for_pattern(0xAA);
    wait_for_pattern(0x55);
    wait_for_pattern(0xFF);
    wait_for_pattern(0x00);
    for (j = 0; j < 40000; j++);
