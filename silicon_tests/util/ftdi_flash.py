...
BLOCK_SIZE_32K = 32 * 1024   # 32KB
BLOCK_SIZE_64K = 64 * 1024   # 64KB


def get_status(device):
    return int.from_bytes(
        device.exchange([CMD_READ_STATUS], 1), byteorder="big"
    )
...
class Led:
    ...
    def toggle(self):
        ...
        # shift to LED GPIO
        output = 0b00000100000000 | self.led << 10
        if self.gpio:
...
slave = spi.get_port(cs=0, freq=0.7e6, mode=0)  # new caravel board
...
# drive MUX select on ACBUS5(14) to 0 to allow the FTDI to program the Flash
gpio.set_direction(0b10010100000000, 0b10010100000000)  # (mask, dir)
gpio.write(0b00000100000000)
...
wcmd = bytearray((CMD_ERASE_SECTOR, (0x0 >> 16) & 0xFF, (0x0 >> 8) & 0xFF, 0x0 & 0xFF))
slave.exchange(wcmd)
# gpio.write(0b00000100000000)
...
with open(file_path, mode="r") as f:
    ...
    while x != "":
        ...
        if nbytes >= 256 or (x != "" and x[0] == "@" and nbytes > 0):
            ...
            # wcmd = bytearray(( CMD_WRITE_ENABLE, CMD_PROGRAM_PAGE,(addr >> 16) & 0xff, (addr >> 8) & 0xff, addr & 0xff))
...
slave.write([0x0B, 0x00])

# led.toggle()
# time.sleep(0.3)
# led.toggle()
# gpio.write(0b10000100000000)
